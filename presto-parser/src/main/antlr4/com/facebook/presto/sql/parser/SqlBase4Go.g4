...
statement
    ...
    | SHOW TABLES ((FROM | IN) qualifiedName)?
        (LIKE pattern=MyString (ESCAPE escape=MyString)?)?                 #showTables
    | SHOW SCHEMAS ((FROM | IN) identifier)?
        (LIKE pattern=MyString (ESCAPE escape=MyString)?)?                 #showSchemas
    | SHOW CATALOGS (LIKE pattern=MyString)?                             #showCatalogs
    ...
    | SHOW FUNCTIONS
        (LIKE pattern=MyString (ESCAPE escape=MyString)?)?                 #showFunctions
...
columnDefinition
    : identifier type_r (NOT NULL)? (COMMENT MyString)? (WITH properties)?
    ;
...
primaryExpression
    ...
    | identifier MyString                                                                   #typeConstructor
    | DOUBLE_PRECISION MyString                                                             #typeConstructor
    ...
    | CAST '(' expression AS type_r ')'                                                     #cast
    | TRY_CAST '(' expression AS type_r ')'                                                 #cast
...
timeZoneSpecifier
    ...
    | TIME ZONE MyString    #timeZoneString
    ;
...
interval
    : INTERVAL sign=(PLUS | MINUS)? MyString from=intervalField (TO to=intervalField)?
    ;
...
type_r
    ...
    | ARRAY '<' type_r '>'
    | MAP '<' type_r ',' type_r '>'
    | ROW '(' identifier type_r (',' identifier type_r)* ')'
    | baseType ('(' typeParameter (',' typeParameter)* ')')?
    | INTERVAL from=intervalField TO to=intervalField
    ;

typeParameter
    : INTEGER_VALUE | type_r
    ;
